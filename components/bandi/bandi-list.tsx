...
// Link alla strategia che porta uno SNAPSHOT del bando nell'URL (?b=): la pagina si ricostruisce
// dai dati nell'URL + DNA/AI senza dipendere dallo store in-memory (che sul serverless si azzera
// tra istanze → 404). Path = ref stabile (fallback dal pool per URL senza snapshot).
function strategyHref(g: Grant): string {
  ...
  return `/bandi/${g.ref ?? g.id}?b=${encodeURIComponent(JSON.stringify(snap))}`
}
